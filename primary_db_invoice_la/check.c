...
int CreateCheckProcess()
{
    ...
    if ((pid = fork())<0) {
        ...
        if (success) {
            OUTPUT_OK;
            LOG(INFO)<<"CHECK_PROCESS running, OK.";
        } else {
            OUTPUT_ERROR;
            LOG(INFO)<<"CHECK_PROCESS running, ERROR.";
        }
...
int HandleCheckProcedure()
{
    ...
    while (1) {
        ...
        for (i=0; i<global_par.system_par.proxy_number; i++) {
            success = CheckControlChannelStateAndReport(i);
        }
...
int CheckControlChannelStateAndReport(int proxy_id)
{
    ...
    if (watchdog_value > THRESHOLD_BROKEN_CONTROL_CHANNEL) {
        ...
        printf("\rThe status of control channel is");
        // OUTPUT_ERROR;
